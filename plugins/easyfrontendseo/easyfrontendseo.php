...
class PlgSystemEasyFrontendSeo extends JPlugin
{
	...
	private function saveDataToTable($document)
	{
		...
		if(mb_strlen($title) > $characters_title)
		{
		...
		if(mb_strlen($description) > $characters_description)
		{
	...
	{
		$automatic_article_length = $this->params->get('automatic_article_length', 20000);
		$keywords_whole_text = $this->cleanString($keywords_whole_text, false, $automatic_article_length);
		$pattern = array('@<[^>]+>@U', '@[,;:!"\.\?]@', '@\s+@');
	 ...
	 * @param integer $length_max
	 *
	 * @return mixed|string
	 */
	private function cleanString($string, $clean_again = false, $length_max)
	{
		...
		if($string_clean === false OR $clean_again == true)
		{
			...
			if(!empty($length_max))
			{
				if(mb_strlen($string) > $length_max)
				{
					$string = mb_strcut($string, 0, $length_max);
				}
			}
			...
			$string_clean = htmlspecialchars($string);
		}
	...
	{
		$automatic_article_length = $this->params->get('automatic_article_length', 20000);
		$content_description_whole_text = $this->cleanString($content_description_whole_text, $clean_again, $automatic_article_length);
		...
		if(mb_strlen($content_description_whole_text) > $content_number_description)
		{
			$content_description = mb_substr($content_description_whole_text, 0, $content_number_description);

			if(mb_substr($content_description_whole_text, $content_number_description, 1) != ' ')
			{
				$content_description = $this->findLastSpaceString($content_description);
			}

			if(!empty($content_description_add_dots))
			{
				if(mb_strlen($content_description) > $content_number_description - 3)
				{
					$content_description = $this->findLastSpaceString($content_description);
				}
	...
	private function buildForm($title, $description, $keywords, $generator, $robots)
	{
		...
		if($this->params->get('style') == 0)
		{
			$output .= '<div id="easyfrontendseo"><h1>'.JText::_('PLG_EASYFRONTENDSEO_PLUGINNAME').'</h1>';
		}
		elseif($this->params->get('style') == 1)
		{
	...
	private function buildButtons($title = '', $description = '', $keywords = '', $generator = '', $robots = '')
	{
		...
		if($this->params->get('style') == 0)
		{
			return '<div id="easyfrontendseo_topbar"><a id="toggle" href="#"><strong>'.JText::_('PLG_EASYFRONTENDSEO_PLUGINNAME_SHORT').'</strong></a>'.$metacheck.'</div>';
		}
